...
const steps = [
  ["01", "You choose the trade", "Send PETE a supported trade instruction through Telegram. You define the direction, entry, stop, targets and risk within the supported command format."],
  ["02", "PETE validates it", "PETE checks that the instruction makes structural sense before it can progress. Invalid trade geometry is rejected rather than silently reinterpreted."],
  ["03", "Your setup is checked", "PETE associates the instruction with the customer, subscription and paired installation while the local Gateway and MT4 route remain part of the execution chain."],
  ["04", "PETE builds the basket", "PETE applies the supplied risk and prepares the execution basket. Multi-target instructions can be represented as multiple broker orders linked to one PETE basket."],
  ["05", "Gateway sends it to MT4", "The PETE Windows Gateway Agent bridges the hosted service to the PETE client running inside your own MT4 terminal on a Windows PC or VPS."],
  ["06", "Your broker executes", "Orders pass through your own MT4 terminal to your own broker account. Broker fills, symbol rules, lot limits, margin and normal execution conditions still apply."],
  ["07", "PETE tracks the basket", "PETE tracks the execution state and basket lifecycle rather than simply sending an instruction and forgetting about it."],
  ["08", "You get feedback", "Telegram becomes the control and feedback surface, showing whether an instruction was accepted, executed, rejected or closed."],
];
    ...
    <main className="min-h-screen bg-[#050505] text-[#f5f5f5]">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            <a href="/trade-gateway" className="hidden text-xs font-bold uppercase tracking-[0.18em] text-zinc-400 hover:text-[#c9a227] sm:block">Trade Gateway</a>
            <a href="/trade-gateway#founding-beta" className="rounded-full bg-[#c9a227] px-5 py-3 text-xs font-black uppercase tracking-widest text-black hover:bg-[#e4c45a]">Founding Beta</a>
          </div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden px-6 pb-24 pt-40">
        ...
        <div className="relative mx-auto max-w-7xl">
          <p className="text-xs font-black uppercase tracking-[0.4em] text-[#c9a227]">PETE Trade Gateway · How it works</p>
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <a href="/trade-gateway#founding-beta" className="rounded-full bg-[#c9a227] px-8 py-4 text-center text-sm font-black uppercase tracking-widest text-black transition hover:bg-[#e4c45a]">Founding Beta · £9.99/month</a>
            <a href="/trade-gateway" className="rounded-full border border-zinc-700 px-8 py-4 text-center text-sm font-bold uppercase tracking-widest transition hover:border-[#c9a227] hover:text-[#c9a227]">Back to Trade Gateway</a>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-[#c9a227]/35 bg-[#0d0d0d] p-8 md:p-12">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-5">
            {["Telegram", "PETE Hosted Core", "Windows Gateway", "MT4", "Your broker"].map((item, index) => (
              <div key={item} className="rounded-2xl border border-zinc-800 bg-black px-5 py-6 text-center">
          ...
          <p className="mt-8 max-w-4xl text-lg leading-8 text-zinc-400">Your broker account remains yours. PETE does not hold your trading funds or operate a pooled trading account. It is the engineered infrastructure between your instruction and your broker.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {steps.map(([number, title, text]) => (
              <article key={number} className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-7">
      ...
      <section className="px-6 py-24">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-8 md:p-10">
            ...
            <p className="mt-6 text-lg leading-8 text-zinc-400">PETE deliberately uses a local Windows or VPS Gateway rather than pretending every MT4 broker account is a direct cloud API. The paired installation, broker binding and MT4 heartbeat form the local execution route between PETE&apos;s hosted control layer and your own terminal.</p>
          ...
          <div className="rounded-3xl border border-[#c9a227]/35 bg-[#12100a] p-8 md:p-10">
            ...
            <ul className="mt-7 space-y-4 text-sm leading-7 text-zinc-400">
              {failClosed.map((item) => (
                <li key={item} className="flex gap-3"><span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#c9a227]" /><span>{item}</span></li>
              ))}
            </ul>
          </div>
        </div>
      ...
      <section className="border-y border-zinc-900 bg-[#080808] px-6 py-24">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-8 md:p-10">
            ...
            <p className="mt-6 text-lg leading-8 text-zinc-400">Founding Beta setup includes a preflight stage before the customer progresses to live broker writes. It checks the local route including MT4 heartbeat, broker/account binding, connectivity, trade permission and online authorization requirements.</p>
          ...
          <div className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-8 md:p-10">
            ...
            <p className="mt-7 leading-8 text-zinc-500">Public Gateway V1 does not choose a market direction on the customer&apos;s behalf. PETE&apos;s role is the execution infrastructure around the instruction.</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {requirements.map((item) => <div key={item} className="rounded-2xl border border-zinc-800 bg-[#0d0d0d] p-6 text-sm leading-7 text-zinc-400">{item}</div>)}
          </div>
          <p className="mt-8 max-w-4xl leading-8 text-zinc-500">A VPS is optional, but useful if you want the Gateway and MT4 available without leaving a home PC running continuously.</p>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-8 md:p-10">
            ...
            <ul className="mt-7 space-y-4 text-lg leading-8 text-zinc-300">
              <li>Customer-directed execution software.</li><li>An MT4 execution bridge.</li><li>A validation and authorization layer.</li><li>A risk-aware basket execution system.</li><li>A trade lifecycle and feedback system.</li><li>Software operating through your own broker account.</li>
            </ul>
          </div>
          <div className="rounded-3xl border border-zinc-800 bg-[#0d0d0d] p-8 md:p-10">
            ...
            <ul className="mt-7 space-y-4 text-lg leading-8 text-zinc-400">
              <li>A signals subscription.</li><li>Investment advice.</li><li>A managed account.</li><li>A profit guarantee.</li><li>A pooled trading account.</li><li>An autonomous public strategy in Gateway V1.</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-zinc-800 bg-[#0d0d0d] p-8 md:p-12">
          ...
          <p className="mt-7 max-w-5xl text-lg leading-8 text-zinc-400">PETE works through the customer&apos;s broker, so broker-specific rules still exist. Symbol names, contract sizes, lot steps, minimum lots, trading permissions, margin requirements and execution conditions can vary. That is one reason the Founding Beta is deliberately being tested across different brokers, PCs and VPS environments.</p>
        </div>
      </section>
      ...
      <section className="border-y border-zinc-900 bg-[#080808] px-6 py-24">
        <div className="mx-auto max-w-5xl rounded-[2rem] border border-[#c9a227]/40 bg-[#0d0d0d] p-8 text-center glow md:p-14">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-zinc-400">The Founding Beta is deliberately small, initially targeting around 5–10 MT4 traders so each onboarding can be observed properly across different brokers, Windows environments and VPS setups.</p>
          ...
          <a href="/trade-gateway#founding-beta" className="mt-9 inline-block rounded-full bg-[#c9a227] px-8 py-4 text-sm font-black uppercase tracking-widest text-black hover:bg-[#e4c45a]">View Founding Beta</a>
        </div>
      </section>
      ...
      <footer className="border-t border-zinc-900 px-6 py-10 text-sm text-zinc-500">
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-2">
          ...
          <div className="md:text-right"><p>© 2026 The Performance Engineer Ltd. All rights reserved.</p><p className="mt-2"><a href="/trade-gateway" className="hover:text-[#c9a227]">PETE Trade Gateway</a></p><p className="mt-2"><a href="/about" className="hover:text-[#c9a227]">About The Performance Engineer</a></p></div>
        </div>
      </footer>
    </main>
  );
